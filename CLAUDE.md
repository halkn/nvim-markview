...
| `/images/<path>`   | GET      | Markdown ファイルのディレクトリを起点とした相対パスの画像ファイルを配信 |
...
- `MIME_TYPES` テーブル / `get_mime_type(path)`: 画像ファイルの拡張子から Content-Type を返す。
...
- `/images/<path>` ハンドラ: `base_dir` からの相対パスでファイルを `vim.uv.fs_open/fstat/fs_read` で読み込み配信。`..` を含むパスは 404 を返す（パストラバーサル防止）。
...
- `apply_inline(s, base_dir)`: すべてのインラインパターンを適用。画像はリンクより先に処理し競合を回避。`base_dir` が指定されている場合、相対パスの画像 src を `/images/<rel>` に書き換える（`https?://`・`/` 始まり・`data:` は変換しない）。
...
- **相対画像パスの変換**: `apply_inline` は `base_dir` が `nil`（バッファが無名の場合）だと変換を行わず、ブラウザがそのままの `src` を使う。保存済みファイルで開いた場合のみ相対パス画像が表示される。
- **`/images/` のパストラバーサル**: `..` を含むパスは即座に 404 を返す。`vim.uv.fs_open` に渡す前のチェックのため、シンボリックリンク経由の迂回には対応していない（ローカル用途では許容範囲）。
